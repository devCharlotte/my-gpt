  ...
  try {
    if (isPackaged) {
      if (isWin) {
        ...
        // exe:       <install>/Miffy Coding Mate.exe
        const installDir = path.resolve(__dirname, "..", "..", "..");
        const exe = path.join(installDir, "Miffy Coding Mate.exe");
        ...
        // deb/dir:  executable is <install>/miffy-coding-mate, same depth as Windows.
        //   __dirname: <install>/resources/app.asar.unpacked/hooks
        //   install:   3 levels up
        ...
        if (appImage) {
          ...
          const installDir = path.resolve(__dirname, "..", "..", "..");
          const exe = path.join(installDir, "miffy-coding-mate");
          spawn(exe, [], { detached: true, stdio: "ignore" }).unref();
        }
      }
    } else {
