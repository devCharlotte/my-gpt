...
function parseClawdImportUrl(rawUrl) {
  ...
  try {
    ...
    throw new Error("invalid Miffy Coding Mate import URL");
  }
  if (parsed.protocol !== "clawd:" && parsed.protocol !== "miffy:") throw new Error("unsupported protocol");

  const action = parsed.hostname || parsed.pathname.replace(/^\/+/, "");
  if (action !== "import-pet") throw new Error(`unsupported mascot import action: ${action || "(missing)"}`);
