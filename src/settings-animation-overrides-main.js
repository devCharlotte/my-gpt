...
const ANIMATION_OVERRIDES_EXPORT_DIALOG_STRINGS = {
  en: {
    ...
    jsonFilter: "Miffy Coding Mate Animation Overrides",
  ...
  zh: {
    ...
    openTitle: "导入动画覆盖",
    defaultName: (ts) => `clawd-animation-overrides-${ts}.json`,
    jsonFilter: "Miffy Coding Mate 动画覆盖",
  ...
  "zh-TW": {
    ...
    defaultName: (ts) => `clawd-animation-overrides-${ts}.json`,
    jsonFilter: "Miffy Coding Mate 動畫與音效自訂設定",
  ...
  ko: {
    ...
    defaultName: (ts) => `clawd-animation-overrides-${ts}.json`,
    jsonFilter: "Miffy Coding Mate 애니메이션 덮어쓰기",
  ...
  ja: {
    ...
    defaultName: (ts) => `clawd-animation-overrides-${ts}.json`,
    jsonFilter: "Miffy Coding Mate アニメーション差し替え",
...
function createSettingsAnimationOverridesMain(options = {}) {
  ...
  async function importAnimationOverrides(event) {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return { status: "error", message: "file is not a Miffy Coding Mate animation overrides export" };
    }
