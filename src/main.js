...
const PREFS_PATH = path.join(app.getPath("userData"), "miffy-coding-mate-prefs.json");
const IS_FIRST_RUN = !fs.existsSync(PREFS_PATH);
const _initialPrefsLoad = prefsModule.load(PREFS_PATH);

// Miffy Coding Mate ships with a personalized first-run profile while keeping
// the upstream preference schema intact for maximum compatibility.
if (IS_FIRST_RUN && _initialPrefsLoad && _initialPrefsLoad.snapshot) {
  _initialPrefsLoad.snapshot.theme = "miffy";
  _initialPrefsLoad.snapshot.openAtLogin = true;
  _initialPrefsLoad.snapshot.autoStartWithClaude = true;
}
...
function hydrateSystemBackedSettings() {
  ...
  try {
    ...
    console.warn("Miffy Coding Mate: failed to read system openAtLogin during hydration:", err && err.message);
  }

  // A fresh installation enables login startup automatically, as promised by
  // the product setup. Existing users keep the OS setting they already chose.
  if (IS_FIRST_RUN && !systemValue) {
    try {
      _writeSystemOpenAtLogin(true);
      systemValue = true;
    } catch (err) {
      console.warn("Miffy Coding Mate: failed to enable first-run login startup:", err && err.message);
    }
  }
...
let _requestedThemeId = _settingsController.get("theme") || "miffy";
...
const _menuCtx = {
  ...
  getActiveThemeId: () => themeRuntime.getActiveThemeId("miffy"),
...
registerSettingsIpc({
  ...
  checkForUpdates,
  aboutHeroSvgPath: path.join(__dirname, "..", "themes", "miffy", "assets", "miffy-idle-follow.svg"),
