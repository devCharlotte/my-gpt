...
const CLAWD_PROTOCOL_SCHEMES = ["miffy"];
const CLAWD_PROTOCOL_SCHEME = CLAWD_PROTOCOL_SCHEMES[0];
...
function extractClawdProtocolUrls(argv) {
  ...
  return argv.filter((arg) => {
    ...
    return CLAWD_PROTOCOL_SCHEMES.some((scheme) => lower.startsWith(`${scheme}:`));
  });
}
...
function createCodexPetMain(options = {}) {
  ...
  const fallbackThemeId = options.fallbackThemeId || "clawd";
  ...
  function getActiveThemeId() {
    ...
    return activeTheme ? activeTheme._id : (settingsController.get("theme") || fallbackThemeId);
  }
  ...
  async function refreshFromSettings() {
    ...
    if (summaryHasActiveCodexPetOrphan(summary, activeId)) {
      const result = await settingsController.applyCommand("setThemeSelection", { themeId: fallbackThemeId });
      if (!result || result.status !== "ok") {
        return {
          status: "error",
          message: (result && result.message) || `failed to switch active orphan Codex Pet theme back to ${fallbackThemeId}`,
          summary,
        };
      }
      switchedToFallback = true;
      const cleanup = syncThemes(fallbackThemeId);
  ...
  function registerProtocolClient() {
    try {
      const results = [];
      if (app.isPackaged) {
        for (const scheme of CLAWD_PROTOCOL_SCHEMES) {
          results.push(app.setAsDefaultProtocolClient(scheme));
        }
      ...
      if (process.argv.includes(REGISTER_PROTOCOL_DEV_ARG) || process.env.CLAWD_REGISTER_PROTOCOL_DEV === "1") {
        ...
        for (const scheme of CLAWD_PROTOCOL_SCHEMES) {
          results.push(app.setAsDefaultProtocolClient(scheme, process.execPath, [appRoot]));
        }
        return results.some(Boolean);
      }
    } catch (err) {
      console.warn("Miffy Coding Mate: failed to register mascot import protocol:", err && err.message);
    }
    return false;
  }
  ...
  function getImportDialogStrings() {
    const all = {
      en: {
        ...
        confirmDetail: (url) => `Miffy Coding Mate will download, validate, and install this pet package before switching to it.\n\n${url}`,
      ...
      zh: {
        ...
        confirmDetail: (url) => `Miffy Coding Mate 会先下载、校验并安装这个宠物包，然后切换到它。\n\n${url}`,
      ...
      "zh-TW": {
        ...
        confirmDetail: (url) => `Miffy Coding Mate 會先下載、驗證並安裝這個寵物套件，然後切換到它。\n\n${url}`,
      ...
      ko: {
        ...
        confirmDetail: (url) => `Miffy Coding Mate가 이 펫 패키지를 다운로드, 검증, 설치한 뒤 전환합니다.\n\n${url}`,
      ...
      ja: {
        ...
        confirmDetail: (url) => `Miffy Coding Mate はこのペットパッケージをダウンロード、検証、インストールしてから切り替えます。\n\n${url}`,
  ...
  function getRemovalDialogStrings() {
    const all = {
      en: {
        ...
        detail: "Miffy Coding Mate will remove the source package from your Codex pets folder and clean up the generated theme. This cannot be undone.",
      },
      zh: {
        uninstall: "卸载",
        cancel: "取消",
        message: (name) => `卸载导入宠物 "${name}"？`,
        detail: "Miffy Coding Mate 会从 Codex pets 文件夹删除源包，并清理生成的主题。此操作不可撤销。",
      },
      "zh-TW": {
        uninstall: "解除安裝",
        cancel: "取消",
        message: (name) => `解除安裝匯入的寵物「${name}」？`,
        detail: "Miffy Coding Mate 會從 Codex pets 資料夾移除來源套件，並清理產生的主題。此操作無法復原。",
      },
      ko: {
        uninstall: "제거",
        cancel: "취소",
        message: (name) => `가져온 펫 "${name}"을(를) 제거할까요?`,
        detail: "Miffy Coding Mate가 Codex pets 폴더의 원본 패키지를 제거하고 생성된 테마를 정리합니다. 이 작업은 되돌릴 수 없습니다.",
      },
      ja: {
        uninstall: "アンインストール",
        cancel: "キャンセル",
        message: (name) => `インポート済みペット "${name}" をアンインストールしますか？`,
        detail: "Miffy Coding Mate は Codex pets フォルダから元パッケージを削除し、生成されたテーマをクリーンアップします。この操作は元に戻せません。",
      },
    };
    return all[getLang()] || all.en;
  }
  ...
  async function materializeAndActivateImportedPet(imported) {
    ...
    if (!generated || !generated.themeId) {
      throw new Error("imported package did not materialize into a Miffy Coding Mate theme");
    }
