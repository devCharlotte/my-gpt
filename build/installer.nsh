!macro customInstall
  ...
  FileOpen $0 "$INSTDIR\.miffy-install-user-home" w
  FileWrite $0 "$PROFILE"
  FileClose $0
!macroend
...
!macro customUnInstall
  StrCpy $1 "$PROFILE"
  IfFileExists "$INSTDIR\.miffy-install-user-home" 0 clawd_node_cleanup_home_done
    FileOpen $0 "$INSTDIR\.miffy-install-user-home" r
    FileRead $0 $1
    FileClose $0
  clawd_node_cleanup_home_done:

  IfFileExists "$INSTDIR\Miffy Coding Mate.exe" 0 clawd_node_cleanup_done
  IfFileExists "$INSTDIR\resources\app.asar.unpacked\hooks\cleanup-integrations.js" 0 clawd_node_cleanup_done
    ...
    nsExec::ExecToLog '"$INSTDIR\Miffy Coding Mate.exe" "$INSTDIR\resources\app.asar.unpacked\hooks\cleanup-integrations.js" --apply --user-home "$1" --source nsis --fail-open'
    Pop $0
